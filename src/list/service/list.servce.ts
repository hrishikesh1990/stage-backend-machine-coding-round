...
export class ListService {
  ...
  async findAll(userId: string, limit: number, offset: number, search: string): Promise<any[]> {
    ...
    const filteredContent = contentDetails.filter((content) =>
      content && (!search || content.title.toLowerCase().includes(search.toLowerCase()))
    );
  ...
  async addToList(userId: string, addToListDto: AddToListDto): Promise<AddToListDto> {
    ...
    const exists = user.myList.some(
      (item) => item.contentId === addToListDto.contentId && item.contentType === addToListDto.contentType,
    );
    if (exists) {
  ...
  async removeFromList(userId: string, removeFromListDto: RemoveFromListDto): Promise<RemoveFromListDto> {
    const user = await this.userModel.findById(userId).exec();
    if (!user) {
      throw new NotFoundException(`User with ID "${userId}" not found`);
    }
    const index = user.myList.findIndex((item) => item.contentId === removeFromListDto.contentId);
    if (index === -1) {
